...
namespace utopia::core {
    ...
    bool safe_add(T first, T second, std::remove_const_t<T> &output) {
    ...
    template<std::integral T>
    bool safe_sub(T first, T second, std::remove_const_t<T> &output) {
#ifdef UTOPIA_USE_GCC | UTOPIA_USE_CLANG
    ...
    /// @return 如果结果没有溢出，则返回true，否则返回false。
    template<std::integral T>
    bool safe_mul(T first, T second, std::remove_const_t<T> &output) {
#ifdef UTOPIA_USE_GCC | UTOPIA_USE_CLANG
    ...
    template<std::integral T>
    bool safe_div(T first, T second, std::remove_const_t<T> &output) {
    ...
    template<std::integral T>
    bool safe_rem(T first, T second, std::remove_const_t<T> &output) {
        if constexpr(std::is_signed_v<T>) {
            if((second == 0) || ((first == LONG_MIN) && (second == -1))) {
                /* Handle error */
                return false;
            }
            else {
